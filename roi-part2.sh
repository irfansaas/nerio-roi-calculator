...
# Create the main ROIAnalysisPage component
...
export function ROIAnalysisPage() {
  ...
  const resetCalculator = () => {
    if (window.confirm('Reset all values to defaults?')) {
      ...
      setUserCount(1000);
      setCurrency('USD');
      // Reset to default values - using spread to create new objects
      setCurrentState({
        physicalServers: 300000,
        storageSystems: 200000,
        networkEquipment: 50000,
...
echo "Part 2 completed: Main ROIAnalysisPage component created"